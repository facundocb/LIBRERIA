function registrar_usuario() {
    ...
    .then(function(respuesta_registro){
        ...
        if(resultado['estado'] == 0){
            result.innerHTML = '<div class="error"> <p class="error_texto"><span class="material-icons">warning</span>No se agrego el usuario</p> </div>'

            for (const key in resultado) {
                cont_errors[key].innerHTML = "";
                cont_errors[key].innerHTML = '<p class="error_p">' + resultado[key] + '</p>';           
            }
        }else{
            result.innerHTML = '<div class="ok"> <p class="ok_texto"><span class="material-icons">done</span>Se agrego el usuario</p> </div>'
        }
